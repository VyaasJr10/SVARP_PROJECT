...
public class HealthDecisionEngine {
    ...
    public HealthDecisionEngine(boolean isHindi) {
        this.isHindi = isHindi;
    }

    // Default constructor — English
    public HealthDecisionEngine() {
        this.isHindi = false;
    }
        ...
        if (isHindi) {
            ...
            if (hasStomachAche && hasDiarrhea)
                return hi("पेट का संक्रमण",
                        ...
                        "ORS या साफ पानी पिते रहें", "हल्का खाना खाएं", "2 दिन बाद भी ठीक न हों तो डॉक्टर से मिलें");
            ...
            for (int i = 0; i < s.size(); i++) {
                symptomList.append(formatSymptomName(s.get(i)));
                if (i < s.size() - 2) symptomList.append(", ");
                else if (i == s.size() - 2) symptomList.append(" और ");
            }
            return hi("कई लक्षण एक साथ",
                    "आपको " + symptomList + " हो रहा है। यह अभी आपातकाल नहीं है, लेकिन अगले 24-48 घंटे ध्यान रखें।",
                    "आराम करें और पानी पिएं", "हर कुछ घंटों में अपनी तबीयत देखें",
                    "बेहतर न हो तो डॉक्टर से मिलें", "नए लक्षण आएं तो नोट करें");

        } else {
            // ── English moderate responses ────────────────────────────────────────
            if (hasFever && hasCough && hasSoreThroat)
            ...
            for (int i = 0; i < s.size(); i++) {
                symptomList.append(formatSymptomName(s.get(i)));
                if (i < s.size() - 2) symptomList.append(", ");
                else if (i == s.size() - 2) symptomList.append(" and ");
            }
            return en("A Few Things Going On",
                    "You're experiencing " + symptomList + ". It's not an emergency, but with a few things happening at once it's a good idea to keep an eye on how you feel.",
